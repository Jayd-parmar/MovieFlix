...
func getCastImageData() -> CastImageModel? {
    let response: CastImageModel?
    do {
        guard let data = CastImageJsonResponse else {
            print("nil returned data \n\n")
            return nil
        }
        response =  try JSONDecoder().decode(CastImageModel.self, from: data)
    } catch(let error){
