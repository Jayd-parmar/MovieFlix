...
func getTvShowDetailsResponse() -> TVShowDetailsModel? {
    let response: TVShowDetailsModel?
    do {
        guard let data = TVShowDetailsJsonResponse else {
            print("nil returned data \n\n")
            return nil
        }
        response =  try JSONDecoder().decode(TVShowDetailsModel.self, from: data)
    } catch(let error){
